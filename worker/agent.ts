...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onStart(): Promise<void> {
    ...
    if (!baseUrl?.startsWith('http') || !apiKey || baseUrl.includes('YOUR') || apiKey === 'YOUR_API_KEY') {
      console.warn('Invalid AI config');
      return;
    }
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    if (method === 'POST' && url.pathname === '/config') {
      ...
      if (this.chatHandler && fullConfig.baseUrl.startsWith('http') && fullConfig.apiKey) {
  ...
  private async handleChatMessage(body: { message: string; stream?: boolean; model?: string }): Promise<Response> {
    ...
    if (!this.chatHandler) {
      return Response.json({
        success: false, 
        error: 'AI configuration missing (CF_AI_BASE_URL & CF_AI_API_KEY required for Cloudflare AI Gateway)'
      }, { status: 503 });
    }
